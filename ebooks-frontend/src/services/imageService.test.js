...
describe('imageService', () => {
    ...
    describe('uploadBookCover', () => {
        ...
        it('rejette un format non autorisé (PDF)', async () => {
            ...
            await expect(imageService.uploadBookCover(file)).rejects.toThrow('Format non supporté');
            expect(api.post).not.toHaveBeenCalled();
        });

        it('rejette un fichier dépassant 5 Mo', async () => {
            const bigFile = createFile('huge.jpg', 'image/jpeg', 6 * 1024 * 1024);
            await expect(imageService.uploadBookCover(bigFile)).rejects.toThrow('dépasse 5 Mo');
            expect(api.post).not.toHaveBeenCalled();
        });
        ...
        it('propage les erreurs réseau', async () => {
            ...
            await expect(imageService.uploadBookCover(file)).rejects.toThrow('Network Error');
        });
    });
